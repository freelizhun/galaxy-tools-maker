...
from . import get_galaxy_connection, load_yaml_file, check_url
from .shed_tools_log import disable_external_library_logging, setup_global_logger
...
class InstallRepositoryManager(object):
    ...
    def install_repositories(self,
        ...
        for repository in filtered_repos.not_installed_repos:
            counter += 1
            if log:
                log_repository_install_start(repository, counter=counter, installation_start=installation_start,
                                             log=log,
            ...
            try:
                ...
                if status == 'Installed':
                    ...
                    raise AssertionError(
                        "Repository name '%s', owner '%s' in unknown status '%s'" % (name, owner, status))
...
def args_to_repos(owner, name, tool_panel_section_id, tool_shed_url):
    # repos=[{'name': 'seqtk_seq', 'owner': 'planemo', 'tool_panel_section_id': 'lammps_manipulation', 'tool_shed_url':
    # 'http://192.168.108.109:9009'}]
    repo = dict(
        owner=owner,
        name=name,
...
def install_tool_from_toolshed(config_f_path,tool_name):
    ...
    name = tool_name
    tool_panel_section_id = tool_list.get('tool_panel_section_id')
    url = tool_list.get('galaxy_tool_shed').get('ip_address')
    tool_shed_url = check_url(url, log=log)

    repos = args_to_repos(owner, name, tool_panel_section_id, tool_shed_url)
    ...
    # Start installing and store the results in install_results.
